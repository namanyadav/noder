...
function isValid(event) {
    return (event.organizer && event.title && event.sDate && event.eDate && event.category && event.capacity
        && event.stAddr && event.state && event.country);
}
...
async function createEvent(eventJson){
    ...
    if(!isValid(event)) throw `Not a valid event ${event}`
